...
count_rgb=$(python3 counting.cpp "$rgb_output_SO" "$config_file")
echo "Final count for RGB: $count_rgb"

count_hsv=$(python3 counting.cpp "$hsv_output_SO" "$config_file")
echo "Final count for HSV: $count_hsv"

count_lab=$(python3 counting.cpp "$lab_output_SO" "$config_file")
